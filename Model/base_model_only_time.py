...
# @Author  : zxl
# @FileName: base_model_only_time.py
...
class base_model_only_time(object):
    ...
    def train(self, sess, batch_data, learning_rate, add_summary=False, global_step=0, epoch=0):
        ...
        output_feed = [self.loss, self.loss_time , self.merged, self.train_op]
    ...
    def metrics_mse(self, sess, batch_data, global_step, topk):
        # set reverse field 0
        # self.init_reserved_field(sess)
        ...
        mse = sess.run([self.loss_time], input_dic)
    ...
    def output(self):
        with tf.name_scope('CrossEntropyLoss'):
            ...
            loss_time
            """
            # self.loss_time = ((self.target[2] - self.predict_time) ** 2)  / self.FLAGS.time_scale  # batch_size,
            self.loss_time = ((self.target[2] - self.predict_time) ** 2) / 1  # batch_size,

            self.loss = regulation_rate * l2_norm +\
                         tf.reduce_mean(self.loss_time)
            # tf.summary.scalar('l2_norm', l2_norm)

            tf.summary.scalar('Training Time Loss', tf.reduce_mean(self.loss_time))
