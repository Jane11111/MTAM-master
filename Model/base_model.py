...
# @Time    : 2020/9/28 9:20
# @Author  : zxl
# @FileName: base_model_no_time.py
...
class base_model(object):
    ...
    def metrics_topK(self, sess, batch_data, global_step, topk):
        ...
        # item_result = tf.matmul(self.predict_behavior_emb, self.embedding.item_emb_lookup_table,transpose_b=True)
        ...
        # indices20 = tf.nn.top_k(item_result, 20).indices
        # indices50 = tf.nn.top_k(item_result, 50).indices

        indices1 = self.indices1
        indices5 = self.indices5
        indices10 = self.indices10
        indices20 = self.indices20
        indices50 = self.indices50

        indices_result1, indices_result5, indices_result10, indices_result20, indices_result50, item_result \
            = sess.run([indices1, indices5, indices10, indices20, indices50, item_result], input_dic)
        result_item = input_dic[self.embedding.target_item_id]
        length = len(batch_data)
        hr_1, ndcg_1, mrr_1, _,_, _ = self.calculate_topK(1, indices_result1, result_item, global_step, length)
        hr_5, ndcg_5, mrr_5, _, _, _ = self.calculate_topK(5, indices_result5, result_item, global_step, length)
        hr_10, ndcg_10, mrr_10, hr_list, ndcg_list_10, mrr_list_10 = self.calculate_topK(10, indices_result10, result_item, global_step,
                                                                    length)
        hr_20, ndcg_20, mrr_20, _, _, _ = self.calculate_topK(20, indices_result20, result_item, global_step, length)
        hr_50, ndcg_50, mrr_50, _, _, _ = self.calculate_topK(50, indices_result50, result_item, global_step, length)

        # print(result_item)

        return hr_1, ndcg_1, mrr_1, hr_5, ndcg_5, mrr_5,  hr_10, ndcg_10,mrr_10, hr_20, ndcg_20,mrr_20, hr_50, ndcg_50, mrr_50, hr_list, ndcg_list_10
    ...
    def calculate_topK(self, k, indices_result, result_item, global_step, length):
        ...
        hr_list = []
        mrr_list = []
        # indices_result  batch_size * k
        # result_item batch_size,
        # length = batch_size
        for one_user_data in indices_result:
            one_user_data = list(one_user_data) # 长度为k
            # print(total_count)
            # print(result_item)
            # print(one_user_data)
            if result_item[total_count] in one_user_data:
                ...
                hr_list.append(1)
                for i in range(len(one_user_data)): #找这个target item在预测的index
        ...
        if len(ndcg_value_list) > 0:
            ...
            avg_ndcg = 0
        if len(mrr_list) >0:
            avg_mrr = float(sum(mrr_list))/length
        else:
            avg_mrr = 0

        return recall_rate, avg_ndcg, avg_mrr, hr_list, ndcg_value_list, mrr_list
    ...
    def metrics_topK_concat(self, sess, batch_data, global_step, topk):
        ...
        hr_1, ndcg_1, mrr_1, _, _, _ = self.calculate_topK(1, indices_result1, result_item, global_step, length)
        hr_5, ndcg_5, mrr_5, _, _, _ = self.calculate_topK(5, indices_result5, result_item, global_step, length)
        hr_10, ndcg_10, mrr_10, hr_list, ndcg_list_10, mrr_list_10 = self.calculate_topK(10, indices_result10,
                                                                                         result_item, global_step,
                                                                                         length)
        hr_20, ndcg_20, mrr_20, _, _, _ = self.calculate_topK(20, indices_result20, result_item, global_step, length)
        hr_50, ndcg_50, mrr_50, _, _, _ = self.calculate_topK(50, indices_result50, result_item, global_step, length)
    ...
    def output(self):
        with tf.name_scope('CrossEntropyLoss'):
            ...
            self.indices20 = tf.nn.top_k(self.item_result, 20).indices
